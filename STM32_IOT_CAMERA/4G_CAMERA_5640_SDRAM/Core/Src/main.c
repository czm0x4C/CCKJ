...
/* USER CODE BEGIN PV */
uint32_t dcmi_data_buff[RGB_DATA_BUFFER_SIZE/4];//JPEG数据缓存buf
...
int main(void)
{
  ...
  if(OV5640_INIT_OK != OV5640_Init())
	...
	HAL_TIM_PWM_Start(&htim2,TIM_CHANNEL_4);			//使能TIM2 CH4 PWM输出
	HAL_TIM_PWM_Start(&htim4,TIM_CHANNEL_1);			//使能TIM4 CH1 PWM输出
