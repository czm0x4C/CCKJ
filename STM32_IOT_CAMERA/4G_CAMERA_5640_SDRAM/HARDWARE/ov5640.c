...
{
	HAL_I2C_Mem_Write(&hi2c1,OV5640_DEVICE_ADDRESS,reg,I2C_MEMADD_SIZE_16BIT,&data,1,HAL_MAX_DELAY);
	return 0;
}
...
{
	uint8_t value = 0;
	HAL_I2C_Mem_Read(&hi2c1,OV5640_DEVICE_ADDRESS,reg,I2C_MEMADD_SIZE_16BIT,&value,1,HAL_MAX_DELAY);
	return value;
}
...
uint8_t OV5640_Init(void)
{
	...
	SCCB_WR_Reg(0x3103,0X11);	//system clock from pad, bit[1]
	SCCB_WR_Reg(0X3008,0X82);	//软复位
	
	HAL_Delay(20);
	
	ov5640CheckId = SCCB_RD_Reg(OV5640_CHIPIDH) << 8 | SCCB_RD_Reg(OV5640_CHIPIDL);
	if(ov5640CheckId != 0x5640)
	{
		return OV5640_INIT_FAIL;
	}		
...
//	OV5640_Sharpness(33);	//自动锐度
	...
	SCCB_WR_Reg(0x5587, 0x00);
	SCCB_WR_Reg(0x5588, 0x01);
	SCCB_WR_Reg(0x3212, 0x13); // end group 3
	SCCB_WR_Reg(0x3212, 0xa3); // launch group 3
	/* 缺省对比度 */
	SCCB_WR_Reg(0x3212, 0x03); // start group 3
	SCCB_WR_Reg(0x5586, 0x20);
	SCCB_WR_Reg(0x5585, 0x00);
	SCCB_WR_Reg(0x3212, 0x13); // end group 3
	SCCB_WR_Reg(0x3212, 0xa3); // launch group 3
	/* 缺省色彩饱和度 */
	SCCB_WR_Reg(0x3212, 0x03); // start group 3
	...
	SCCB_WR_Reg(0x5384, 0x1a);
	SCCB_WR_Reg(0x5385, 0x66);
	SCCB_WR_Reg(0x5386, 0x80);
	SCCB_WR_Reg(0x5387, 0x82);
	SCCB_WR_Reg(0x5388, 0x80);
	SCCB_WR_Reg(0x5389, 0x02);
	SCCB_WR_Reg(0x538b, 0x98);
	SCCB_WR_Reg(0x538a, 0x01);
	SCCB_WR_Reg(0x3212, 0x13); // end group 3
	SCCB_WR_Reg(0x3212, 0xa3); // launch group 3
	/* EV Standard */
	SCCB_WR_Reg(0x3a0f, 0x38);
	SCCB_WR_Reg(0x3a10, 0x30);
	SCCB_WR_Reg(0x3a11, 0x61);
	SCCB_WR_Reg(0x3a1b, 0x38);
	SCCB_WR_Reg(0x3a1e, 0x30);
	SCCB_WR_Reg(0x3a1f, 0x10);
	/* Auto 自动 */
	SCCB_WR_Reg(0x3212, 0x03); // start group 3
	SCCB_WR_Reg(0x3406, 0x00);
	SCCB_WR_Reg(0x3400, 0x04);
	SCCB_WR_Reg(0x3401, 0x00);
	SCCB_WR_Reg(0x3402, 0x04);
	SCCB_WR_Reg(0x3403, 0x00);
	SCCB_WR_Reg(0x3404, 0x04);
	SCCB_WR_Reg(0x3405, 0x00);
	SCCB_WR_Reg(0x3212, 0x13); // end group 3
	SCCB_WR_Reg(0x3212, 0xa3); // lanuch group 3
	/* Normal (off) 正常 */
	SCCB_WR_Reg(0x3212, 0x03); // start group 3
	SCCB_WR_Reg(0x5580, 0x06);
	SCCB_WR_Reg(0x5583, 0x40); // sat U
	SCCB_WR_Reg(0x5584, 0x10); // sat V
	SCCB_WR_Reg(0x5003, 0x08);
	SCCB_WR_Reg(0x3212, 0x13); // end group 3
	SCCB_WR_Reg(0x3212, 0xa3); // launch group 3

//	uint16_t temp = SCCB_RD_Reg(0x3a00);
//	temp = temp | 0x04;
//	SCCB_WR_Reg(0x3a00, temp);
	/* 关闭夜景模式 */
	uint16_t temp = SCCB_RD_Reg(0x3a00);
	temp = temp & 0xfb;
	SCCB_WR_Reg(0x3a00, temp);
	...
	return OV5640_INIT_OK;
}

void ov5640Normal(void)
{
	/* 正常 */
	uint16_t reg3820 = SCCB_RD_Reg(0x3820);
	reg3820 = reg3820 & 0xf9; // flip off
	SCCB_WR_Reg(0x3820, reg3820);
	uint16_t reg3821 = SCCB_RD_Reg(0x3821);
	reg3821 = reg3821 & 0xf9; // mirror off
	SCCB_WR_Reg(0x3821, reg3821);
}
