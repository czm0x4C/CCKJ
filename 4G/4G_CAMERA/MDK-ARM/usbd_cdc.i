...
#line 60 "../Middlewares/ST/STM32_USB_Device_Library/Class/CDC/Src/usbd_cdc.c"
#line 61 "../Middlewares/ST/STM32_USB_Device_Library/Class/CDC/Src/usbd_cdc.c"
...
__align(4) static uint8_t USBD_CDC_DeviceQualifierDesc[0x0AU]  =
{
  0x0AU,
  0x06U,
  0x00,
...
USBD_ClassTypeDef  USBD_CDC =
{
  ...
  0,
  0,
#line 160 "../Middlewares/ST/STM32_USB_Device_Library/Class/CDC/Src/usbd_cdc.c"
...
__align(4) static uint8_t USBD_CDC_CfgDesc[67U]  =
{
...
static uint8_t CDCInEpAdd = 0x81U;
static uint8_t CDCOutEpAdd = 0x01U;
static uint8_t CDCCmdEpAdd = 0x82U;
...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  hcdc = (USBD_CDC_HandleTypeDef *)(void *)USBD_static_malloc(sizeof(USBD_CDC_HandleTypeDef));

  if (hcdc == 0)
  {
    pdev->pClassDataCmsit[pdev->classId] = 0;
...
#line 312 "../Middlewares/ST/STM32_USB_Device_Library/Class/CDC/Src/usbd_cdc.c"

  if (pdev->dev_speed == USBD_SPEED_HIGH)
  {
     
    (void)USBD_LL_OpenEP(pdev, CDCInEpAdd, 0x02U,
                         512U);

    pdev->ep_in[CDCInEpAdd & 0xFU].is_used = 1U;

     
    (void)USBD_LL_OpenEP(pdev, CDCOutEpAdd, 0x02U,
                         512U);

    pdev->ep_out[CDCOutEpAdd & 0xFU].is_used = 1U;

     
    pdev->ep_in[CDCCmdEpAdd & 0xFU].bInterval = 0x10U;
  }
  else
  {
  ...
  (void)USBD_LL_OpenEP(pdev, CDCCmdEpAdd, 0x03U, 8U);
...
#line 400 "../Middlewares/ST/STM32_USB_Device_Library/Class/CDC/Src/usbd_cdc.c"
  ...
  if (pdev->pClassDataCmsit[pdev->classId] != 0)
  {
    ((USBD_CDC_ItfTypeDef *)pdev->pUserData[pdev->classId])->DeInit();
    (void)USBD_static_free(pdev->pClassDataCmsit[pdev->classId]);
    pdev->pClassDataCmsit[pdev->classId] = 0;
    pdev->pClassData = 0;
  }
    ...
    case 0x20U:
      if (req->wLength != 0U)
      {
        if ((req->bmRequest & 0x80U) != 0U)
        {
          ...
          len = (((0x7U) < (req->wLength)) ? (0x7U) : (req->wLength));
          (void)USBD_CtlSendData(pdev, (uint8_t *)hcdc->data, len);
        }
        else
        {
          hcdc->CmdOpCode = req->bRequest;
          hcdc->CmdLength = (uint8_t)(((req->wLength) < (64U)) ? (req->wLength) : (64U));
...
static uint8_t USBD_CDC_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
      ...
      ((pdev->ep_in[epnum & 0xFU].total_length % hpcd->IN_ep[epnum & 0xFU].maxpacket) == 0U))
  {
    ...
    (void)USBD_LL_Transmit(pdev, epnum, 0, 0U);
  }
  else
  {
    hcdc->TxState = 0U;

    if (((USBD_CDC_ItfTypeDef *)pdev->pUserData[pdev->classId])->TransmitCplt != 0)
    {
...
static uint8_t USBD_CDC_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  if ((pdev->pUserData[pdev->classId] != 0) && (hcdc->CmdOpCode != 0xFFU))
  {
...
static uint8_t *USBD_CDC_GetFSCfgDesc(uint16_t *length)
{
  USBD_EpDescTypeDef *pEpCmdDesc = USBD_GetEpDesc(USBD_CDC_CfgDesc, 0x82U);
  USBD_EpDescTypeDef *pEpOutDesc = USBD_GetEpDesc(USBD_CDC_CfgDesc, 0x01U);
  USBD_EpDescTypeDef *pEpInDesc = USBD_GetEpDesc(USBD_CDC_CfgDesc, 0x81U);

  if (pEpCmdDesc != 0)
  {
    pEpCmdDesc->bInterval = 0x10U;
  }

  if (pEpOutDesc != 0)
  {
    pEpOutDesc->wMaxPacketSize = 64U;
  }

  if (pEpInDesc != 0)
  {
    pEpInDesc->wMaxPacketSize = 64U;
  }
...
static uint8_t *USBD_CDC_GetHSCfgDesc(uint16_t *length)
{
  ...
  if (pEpOutDesc != 0)
  {
    pEpOutDesc->wMaxPacketSize = 512U;
  }

  if (pEpInDesc != 0)
  {
    pEpInDesc->wMaxPacketSize = 512U;
  }
