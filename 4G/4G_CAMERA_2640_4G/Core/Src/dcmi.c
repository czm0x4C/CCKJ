...
//	DCMI->IER=0x0;										
	...
	__HAL_DCMI_ENABLE_IT(&hdcmi,DCMI_IT_VSYNC);		//开启帧中断
...
//	HAL_DMAEx_MultiBufferStart(&hdma_dcmi,(uint32_t)&DCMI->DR,(uint32_t)&dcmi_line_buf[0],(uint32_t)&dcmi_line_buf[1],jpeg_line_size);//开启双缓冲
//	__HAL_DMA_ENABLE_IT(&hdma_dcmi,DMA_IT_TC);    //开启传输完成中断
//	HAL_NVIC_SetPriority(DMA1_Stream0_IRQn,0,0);        //DMA中断优先级
...
////捕获到一帧图像处理函数HAL_DCMI_VsyncEventCallback
////hdcmi:DCMI句柄HAL_DCMI_FrameEventCallback
//void HAL_DCMI_VsyncEventCallback(DCMI_HandleTypeDef *hdcmi)
//{
////	jpeg_data_process();//jpeg数据处理 
//    //重新使能帧中断,因为HAL_DCMI_IRQHandler()函数会关闭帧中断
//  __HAL_DCMI_ENABLE_IT(hdcmi,DCMI_IT_VSYNC);//DCMI_IT_VSYNC DCMI_IT_FRAME
//}
/* USER CODE END 1 */
